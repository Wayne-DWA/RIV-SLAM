...
public:
  ...
  KeyframeUpdater(ros::NodeHandle& pnh) : is_first(true), prev_keypose(Eigen::Isometry3d::Identity()) {
    keyframe_delta_trans = pnh.param<double>("keyframe_delta_trans", 0.15 );
    ...
    keyframe_min_size = pnh.param<int>("keyframe_min_size", 20);//unused
...
private:
  ...
  std::size_t keyframe_min_size;  //unused
