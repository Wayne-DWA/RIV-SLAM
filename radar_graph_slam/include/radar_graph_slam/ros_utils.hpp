...
static geometry_msgs::Transform isometry2transform(const Eigen::Isometry3d& iso) {
  geometry_msgs::Transform transf;
  transf.translation.x = iso.translation().x();
  transf.translation.y = iso.translation().y();
  transf.translation.z = iso.translation().z();
  Eigen::Quaterniond quat(iso.linear());
  transf.rotation.w = quat.w();
  transf.rotation.x = quat.x();
  transf.rotation.y = quat.y();
  transf.rotation.z = quat.z();
  return transf;
}
