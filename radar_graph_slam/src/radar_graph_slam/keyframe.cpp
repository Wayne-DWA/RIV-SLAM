...
KeyFrame::KeyFrame(const size_t index, const ros::Time& stamp, const Eigen::Isometry3d& odom_scan2scan, double accum_distance, const pcl::PointCloud<PointT>::ConstPtr& cloud,const Eigen::Vector3d& velocity,const Eigen::Isometry3d& riv_pose) : 
  index(index), stamp(stamp), odom_scan2scan(odom_scan2scan), accum_distance(accum_distance), cloud(cloud),velocity(velocity),riv_pose(riv_pose) , node(nullptr) {}
