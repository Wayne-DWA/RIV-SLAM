...
Eigen::MatrixXd InformationMatrixCalculator::calc_information_matrix(const pcl::PointCloud<PointT>::ConstPtr& cloud1, const pcl::PointCloud<PointT>::ConstPtr& cloud2, const Eigen::Isometry3d& relpose) const {
  ...
  float w_x = 1.0e-8*weight(var_gain_a, fitness_score_thresh, min_var_x, max_var_x, fitness_score); // 0.0001 is a scaling factor
  float w_q = 1.0e-8*weight(var_gain_a, fitness_score_thresh, min_var_q, max_var_q, fitness_score);
