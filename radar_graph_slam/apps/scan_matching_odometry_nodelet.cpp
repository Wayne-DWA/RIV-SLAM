...
public:
  ...
  virtual void onInit() {
    ...
    {
      points_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(mt_nh, "/underfloor_filtered_points", 32)); 
    }
    else points_sub.reset(new message_filters::Subscriber<sensor_msgs::PointCloud2>(mt_nh, "/filtered_points", 32));
    ...
    imu_sub = nh.subscribe("/imu_transformed", 1024, &ScanMatchingOdometryNodelet::imu_callback, this);
    ...
    odom_inc_pub = nh.advertise<nav_msgs::Odometry>("/odom_inc", 32);
...
private:
  ...
  void pointcloud_callback(const geometry_msgs::TwistWithCovarianceStampedConstPtr& twistMsg, const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    // this is where ego vel be appied
    ...
    if(private_nh.param<bool>("enable_under_floor_removal", false))
    {
      read_until->frame_id = "/underfloor_filtered_points";
    }
    else read_until->frame_id = "/filtered_points"; 
    read_until_pub.publish(read_until);
  }
  ...
  Eigen::Matrix4d matching(const ros::Time& stamp, const pcl::PointCloud<PointT>::ConstPtr& cloud) {
    ...
    // geometry_msgs::Pose relativePose = isometry2pose(Eigen::Isometry3d(registration_s2s->getFinalTransformation().cast<double>()));
    // ROS_INFO("scanmatching relative pose x y z: %f, %f, %f", relativePose.position.x, relativePose.position.y, relativePose.position.z);
    ...
    if(keyframe_updater->decide(Eigen::Isometry3d(odom_s2s_now), stamp)) {
      ...
      KeyFrame::Ptr keyframe(new KeyFrame(keyframe_index, stamp, Eigen::Isometry3d(odom_s2s_now.cast<double>()), accum_d, cloud,Eigen::Vector3d::Zero(),Eigen::Isometry3d::Identity()));
      keyframe_index ++;
  ...
  void publish_odometry(const ros::Time& stamp, const std::string& father_frame_id, const std::string& child_frame_id, const Eigen::Matrix4d& pose_in, const geometry_msgs::TwistWithCovariance twist_in) {
    ...
    nav_msgs::Odometry odom_inc;
    odom_inc.header.stamp = stamp;
    odom_inc.header.frame_id = father_frame_id;   // frame: /odom
    odom_inc.child_frame_id = "odom_mapping";
    Eigen::Isometry3d this_odom = Eigen::Isometry3d(pose_in);
    Eigen::Isometry3d odom_inc_ = last_odom.inverse() * this_odom;

    Eigen::Quaterniond quat(odom_inc_.linear());
    Eigen::Vector3d trans = odom_inc_.translation();

    odom_inc.pose.pose.position.x = trans.x();
    odom_inc.pose.pose.position.y = trans.y();
    odom_inc.pose.pose.position.z = trans.z();
    odom_inc.pose.pose.orientation.w = quat.w();
    odom_inc.pose.pose.orientation.x = quat.x();
    odom_inc.pose.pose.orientation.y = quat.y();
    odom_inc.pose.pose.orientation.z = quat.z();
    odom_inc_pub.publish(odom_inc);
  }
  ...
  void publish_scan_matching_status(const ros::Time& stamp, const std::string& frame_id, pcl::PointCloud<pcl::PointXYZI>::ConstPtr aligned, const std::string& msf_source, const Eigen::Isometry3d& msf_delta) {
    ...
    // ROS_ERROR("scanmatching relative pose x y z: %f, %f, %f", status.relative_pose.position.x, status.relative_pose.position.y, status.relative_pose.position.z);
    // ROS_WARN("scanmatching relative pose x y z: " << status.relative_pose.position.x << " " << status.relative_pose.position.y << " " << status.relative_pose.position.z);
...
private:
  ...
  bool enable_under_floor_removal;

  // std::unique_ptr<GraphSLAM> graph_slam;
  // std::unique_ptr<InformationMatrixCalculator> inf_calclator;
  
  ros::Publisher odom_pub;
  ros::Publisher odom_inc_pub;
