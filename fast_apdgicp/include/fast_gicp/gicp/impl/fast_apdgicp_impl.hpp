...
#pragma omp parallel for num_threads(num_threads_) firstprivate(k_indices, k_sq_dists) schedule(guided, 8)
  for (int i = 0; i < input_->size(); i++) {
    ...
    double point_AoA = atan2(pt.x, sqrt(pt.y * pt.y + pt.z * pt.z));
    double s_x = dist * distance_variance_ / 400; // 0.00215
    double s_y = dist * sin(azimuth_variance_ / 180 * M_PI) / cos(point_AoA);
    double s_z = dist * sin(elevation_variance_ / 180 * M_PI) / cos(point_AoA);
